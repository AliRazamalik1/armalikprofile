...
{
	global $pagenow;
	if (get_option('portfoliocanfit5') || $pagenow == 'themes.php') {
		return;
	}
	$class = 'eye-notice notice notice-warning is-dismissible';
	$message = __('<strong> Hi Buddy!! Now You are using the Free version of Portfolio Canvas theme.<br> Portfolio Canvas PRO version is now available with an auto-setup feature!! Only active the theme and follow auto-setup then your site will be ready with lots of advanced features. Now create your site like a pro with only a few clicks!!!! So why late get pro now </strong> ', 'portfolio-canvas');
	$url1 = esc_url('https://wpthemespace.com/product/portfolio-canvas-pro/');
	$url3 = esc_url('https://wpthemespace.com/product/portfolio-canvas-pro/?add-to-cart=6998');

	printf('<div class="%1$s" style="padding:10px 15px 20px;text-transform:uppercase"><p>%2$s</p><a target="_blank" class="button button-primary" href="%3$s" style="margin-right:10px">' . __('Portfolio Canvas Pro Details', 'portfolio-canvas') . '</a><a target="_blank" class="button button-primary" href="%4$s" style="margin-right:10px">' . __('Upgrade Pro', 'portfolio-canvas') . '</a><button class="button button-info btnend" style="margin-left:10px">' . __('Dismiss the notice', 'portfolio-canvas') . '</button></div>', esc_attr($class), wp_kses_post($message), $url1, $url3);
}
...
function portfolio_canvas_new_optins_texts_init()
{
	if (isset($_GET['xbnotice']) && $_GET['xbnotice'] == 1) {
		delete_option('portfoliocanfit4');
		update_option('portfoliocanfit5', 1);
	}
}
